...
void MenuStage::OnEnter()
{
    ...
    LabelWidget* lw = LabelWidget::Create(0, 20, FONT_BIGGER, "BubbleWorld", BWCOLOR_BUZNABLUE);
    ...
    // username label
    lw = LabelWidget::Create(0, 0, FONT_MAIN, "Username");
    lw->SetPositionXLambda([lw](int32_t win_w, int32_t) {
        return win_w / 2 - lw->GetWidth() / 2;
    });
    ...
    TextFieldWidget* wg = TextFieldWidget::Create(20, 130, 400, FONT_MAIN, "");
    ...
    // password label
    lw = LabelWidget::Create(0, 200, FONT_MAIN, "Password");
    lw->SetPositionXLambda([lw](int32_t win_w, int32_t) {
        return win_w / 2 - lw->GetWidth() / 2;
    });
    lw->SetPositionYLambda([lw](int32_t, int32_t win_h) {
    ...
    wg = TextFieldWidget::Create(0, 230, 400, FONT_MAIN, "");
    ...
    // login button
    ButtonWidget* bw = ButtonWidget::Create(0, 280, FONT_MAIN, "LOGIN");
...
void MenuStage::OnUIAction(uint32_t elementId, UIActionType actionId)
{
    ...
    if (elementId == ELEMENT_LOGIN_BTN && actionId == UIACTION_MOUSE_CLICK_LEFT)
    {
        ...
        const char* username = tfw->GetText();
        tfw = (TextFieldWidget*)sDrawing->FindUIWidgetById(ELEMENT_PASSWORD_FIELD);
        const char* password = tfw->GetText();

        // secure length
        if (strlen(username) == 0)
        {
            sDrawing->AddUIWidget(SplashMessageWidget::Create(SPLASH_TYPE_ERROR, 500, FONT_MAIN, "Enter your username!"));
            return;
        }
        if (strlen(password) == 0)
        {
            sDrawing->AddUIWidget(SplashMessageWidget::Create(SPLASH_TYPE_ERROR, 500, FONT_MAIN, "Enter your password!"));
            return;
        }
...
void MenuStage::OnGlobalAction(GlobalActionIDs actionId, void* actionParam)
{
    switch (actionId)
    {
        case GA_CONNECTION_START:
            sDrawing->AddUIWidget(SplashMessageWidget::Create(SPLASH_TYPE_NORMAL, 500, FONT_MAIN, "Connecting...", false));
            break;
        case GA_CONNECTION_CONNECTED:
        {
            sDrawing->AddUIWidget(SplashMessageWidget::Create(SPLASH_TYPE_NORMAL, 500, FONT_MAIN, "Authenticating...", false));
            ...
            sGameplay->Login(username, password);
            break;
        }
        case GA_CONNECTION_FETCHING:
            sDrawing->AddUIWidget(SplashMessageWidget::Create(SPLASH_TYPE_NORMAL, 500, FONT_MAIN, "Fetching data...", false));
        ...
        case GA_CONNECTION_UNABLE_TO_CONNECT:
            sDrawing->AddUIWidget(SplashMessageWidget::Create(SPLASH_TYPE_ERROR, 500, FONT_MAIN, "Unable to connect to server! Please, check your internet connection and try again."));
            break;
        case GA_CONNECTION_INVALID_USER:
            sDrawing->AddUIWidget(SplashMessageWidget::Create(SPLASH_TYPE_ERROR, 500, FONT_MAIN, "Invalid username"));
            break;
        case GA_CONNECTION_INVALID_PASSWORD:
            sDrawing->AddUIWidget(SplashMessageWidget::Create(SPLASH_TYPE_ERROR, 500, FONT_MAIN, "Invalid password"));
            break;
        case GA_CONNECTION_INCOMPATIBLE_VERSION:
            sDrawing->AddUIWidget(SplashMessageWidget::Create(SPLASH_TYPE_ERROR, 500, FONT_MAIN, "Incompatible client version, please update"));
            break;
        case GA_CONNECTION_BANNED:
            sDrawing->AddUIWidget(SplashMessageWidget::Create(SPLASH_TYPE_ERROR, 500, FONT_MAIN, "Your account has been banned!"));
            break;
    }
}
